      ...
      <div class="card-header">
        ...
        <div class="card-search">
          <el-row :gutter="12">
            ...
            <el-col :span="3">
              <el-input :prefix-icon="Search" v-model="stuno" @keyup.enter.native="search" placeholder="学号搜索（回车）" />
            </el-col>
            <el-col :span="3">
              <el-input :prefix-icon="Search" v-model="name" @keyup.enter.native="search" placeholder="姓名搜索（回车）" />
            </el-col>
            <el-col :span="2"
    ...
    <div class="table-box">
      <el-table row-key="id" element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column label="学生姓名" prop="name">
          <template #default="scope">
            <span>{{ scope.row.students.studentName }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column>
          ...
          <template #default="scope">
            <div class="edit-score" v-if="scope.row.edit">
              <el-input clearable placeholder="请输入" v-model="scope.row.score"></el-input>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-popconfirm confirm-button-text="确定" cancel-button-text="取消" :icon="Delete" icon-color="#626AEF"
              :title="'确定删除学生名为“' + scope.row.students.studentName + '”的成绩吗？'" @confirm="delScores(scope.row.scoreId)">
              <template #reference>

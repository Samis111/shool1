    ...
    <template #header>
      <div class="card-header">
        ...
        <div class="card-search">
          <el-row :gutter="10">
            <el-col :span="10">
              <el-input :prefix-icon="Search" v-model="searchValue" @keyup.enter.native="search" placeholder="关键字搜索" />
            </el-col>
            <el-col :span="11">
              <div class="my-button">
                <el-button plain style="width: 100%;" color="#2fa7b9" @click="addRole">添加院系</el-button>
      ...
      <el-table element-loading-text="数据加载中..." v-loading="loading" :data="tableData"
        ...
        <el-table-column label="院系名称">
          <template #default="scope">
            <el-tooltip :content="scope.row.departmentName" palacement="top" effect="light">
              <span class="highlight">{{ scope.row.departmentName }}</span>
            </el-tooltip>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作">
          <template #default="scope">
            <el-button size="small" @click="editRole(scope.row.departmentId)" style="margin: 0 0 10px 10px;">编辑</el-button>
            <el-popconfirm width="200px" confirm-button-text="确定" cancel-button-text="取消" :icon="Delete"
              icon-color="#626AEF" :title="'确定删除角色名为“' + scope.row.departmentName + '”的角色吗？'"
              @confirm="delRole(scope.row.departmentId)">
              <template #reference>
...
// 删除角色信息
const delRole = async (departmentId: number) => {
  const { data } = await deleteRoleApi(departmentId)
  if (data.code === 200) {
